...
- (void) requestInitData:(NSDate *)date{
    NSString *url = [NSString stringWithFormat:HOTNEWS_INIT, @"1", [Util NSDateToString:self.curDate], 10];
...
- (void) requestNewData:(NSDate *)date{
    NSString *url = [NSString stringWithFormat:HOTNEWS_NEW, @"1", [Util NSDateToString:self.curDate], 10];
    url = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSLog(@"%@, %@", self.curDate, url);
    [NetworkUtil JSONDataWithUrl:url success:^(id json){
        NSDictionary *responseJson = json;
...
- (void) requestOldData:(NSDate *)date{
    NSString *url = [NSString stringWithFormat:HOTNEWS_OLD, @"1", [Util NSDateToString:self.lastDate], 10];
    url = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
