...
@interface EventEndedListController ()

@end

@implementation EventEndedListController
...
- (void) requestInitData:(NSDate *)date{
    NSString *url = [NSString stringWithFormat:EVENT_FIND_ENDED_INIT,[Util NSDateToString:self.curDate], 10];
    ...
    [NetworkUtil JSONDataWithUrl:url success:^(id json){
        ...
        self.curDate = [Util NSDateFromString:[[self.infoArr objectAtIndex:0] objectForKey:@"endAt"]];
        if (self.infoArr.count == 0 ) {
            self.lastDate = self.curDate;
        }else{
            self.lastDate = [Util NSDateFromString:[[self.infoArr objectAtIndex:(self.infoArr.count-1)] objectForKey:@"endAt"]];
        }
        [self.tableView reloadData];
    }fail:^(void){
        NSLog(@"NETWORK ERROR!!!");
...
- (void) requestNewData:(NSDate *)date{
    NSString *url = [NSString stringWithFormat:EVENT_FIND_ENDED_NEW, [Util NSDateToString:self.curDate], 10];
    url = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSLog(@"%@, %@", self.curDate, url);
    [NetworkUtil JSONDataWithUrl:url success:^(id json){
        NSDictionary *responseJson = json;
...
- (void) requestOldData:(NSDate *)date{
    NSString *url = [NSString stringWithFormat:EVENT_FIND_ENDED_OLD, [Util NSDateToString:self.lastDate], 10];
    url = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
